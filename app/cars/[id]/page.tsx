...
function getImageUrl(image: string | undefined) {
  if (!image) return "https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=1200&q=80"
...
function CommentSection({ carId }: { carId: string }) {
  ...
  const currentUserId = typeof window !== "undefined" ? JSON.parse(localStorage.getItem("user") || "null")?._id : null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/comments/${carId}`, {
        method: "POST",
          ...
          <Card key={comment._id} className="bg-card border-border">
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                <Avatar>
                  <AvatarImage src={getAvatarUrl(comment.author?.profileImage)} alt={comment.author?.username} />
...
export default function CarDetailPage() {
  ...
  const toggleLike = async () => {
    const token = localStorage.getItem("accessToken")
    if (!token) return
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/cars/${carId}/like`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    })
    if (res.ok) {
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4 mb-4">
                      <Avatar className="w-16 h-16">
                        <AvatarImage src={getAvatarUrl(car.owner?.profileImage)} alt={car.owner?.username} />
                        <AvatarFallback>{car.owner?.username?.charAt(0)}</AvatarFallback>
                      </Avatar>
                      <div>
                        <h3 className="font-semibold text-foreground">{car.owner?.username}</h3>
                        <p className="text-sm text-muted-foreground">
                          Member since {car.owner?.createdAt ? new Date(car.owner.createdAt).getFullYear() : "—"}
                        </p>
                      </div>
                    </div>
